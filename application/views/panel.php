<script src="<?php echo base_url()?>bootstrap/amcharts/amcharts.js"></script>
<script src="<?php echo base_url()?>bootstrap/amcharts/pie.js"></script>
<link rel="stylesheet" href="<?php echo base_url()?>bootstrap/amcharts/plugins/export/export.css" type="text/css" media="all" />
<script src="<?php echo base_url()?>bootstrap/amcharts/themes/light.js"></script>
<script src="<?php echo base_url()?>bootstrap/js/graficas_zona.js" type="text/javascript"></script>

<div class="container-fluid">    
  <div class="row">
    <div class="col-sm-6">
      <div class="panel panel-primary">
        <div class="panel-heading">Zona</div>
        <div class="panel-body"></div>
        <div class="panel-footer">Zonas afectadas por el sismo</div>
      </div>
    </div>
    <div class="col-sm-6"> 
      <div class="panel panel-primary">
        <div class="panel-heading">Delegaciones</div>
        <div class="panel-body"><img src="https://placehold.it/150x80?text=IMAGE" class="img-responsive" style="width:100%" alt="Image"></div>
        <div class="panel-footer">Delegaciones con afectaciones por le sismo</div>
      </div>
    </div>
  </div>
</div><br>

<div class="container">    
  <div class="row">
    <div class="col-sm-4">
      <div class="panel panel-success">
        <div class="panel-heading">Tipo de daño</div>
        <div class="panel-body"><img src="https://placehold.it/150x80?text=IMAGE" class="img-responsive" style="width:100%" alt="Image"></div>
        <div class="panel-footer">Habitaciones con algun tipo de afectacion</div>
      </div>
    </div>
    <div class="col-sm-4"> 
      <div class="panel panel-success">
        <div class="panel-heading">Nivel de daño</div>
        <div class="panel-body"><img src="https://placehold.it/150x80?text=IMAGE" class="img-responsive" style="width:100%" alt="Image"></div>
        <div class="panel-footer">Habitaciones con algun nivel de daño</div>
      </div>
    </div>
    <div class="col-sm-4"> 
      <div class="panel panel-success">
        <div class="panel-heading">Habitaciones</div>
        <div class="panel-body"><img src="https://placehold.it/150x80?text=IMAGE" class="img-responsive" style="width:100%" alt="Image"></div>
        <div class="panel-footer">Tipo de habitacion con daños</div>
      </div>
    </div>
  </div>
</div><br><br>